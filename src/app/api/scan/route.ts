...
export async function POST(req: Request) {
  ...
  const universe: Universe = body.universe ?? 'NIFTY100'
  ...
  } catch (_e) { redis = null /* proceed without cache */ }
  ...
  const stream = new ReadableStream({
    async start(ctrl) {
      ...
      try {
        ...
        if (redis) {
          try { await redis.setex('scan:' + universe, 600, JSON.stringify(result)) } catch (_e) { /* skip */ }
        }
