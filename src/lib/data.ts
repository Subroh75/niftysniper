...
// ─── NIFTY 50 — exact 50 constituents ────────────────────────────────────────
...
// ─── NIFTY 200 extra — 150 stocks NOT already in NIFTY50 ─────────────────────
const NIFTY200_EXTRA: Record<string, string> = {
  ...
  'EMAMILTD.NS':'FMCG',        'GAIL.NS':'Energy',          'IGL.NS':'Energy',
  'PETRONET.NS':'Energy',      'HINDPETRO.NS':'Energy',     'IOC.NS':'Energy',
  'HPCL.NS':'Energy',          'NMDC.NS':'Metals',          'VEDL.NS':'Metals',
  'SAIL.NS':'Metals',          'NATIONALUM.NS':'Metals',    'HINDZINC.NS':'Metals',
  'NHPC.NS':'Utilities',       'CESC.NS':'Utilities',       'TORNTPOWER.NS':'Utilities',
  'ADANIGREEN.NS':'Utilities', 'JSWENERGY.NS':'Utilities',  'SUZLON.NS':'Utilities',
  'HAL.NS':'Industrials',      'BHEL.NS':'Industrials',
}

// ─── NIFTY 500 extra — stocks NOT already in NIFTY200 ────────────────────────
const NIFTY500_EXTRA: Record<string, string> = {
  ...
  'DCBBANK.NS':'Financials',   'MAHINDCIE.NS':'Auto',       'SUNDRMFAST.NS':'Auto',
  ...
  'SUNTV.NS':'Consumer',       'ZEEL.NS':'Consumer',        'BDL.NS':'Industrials',
  'BEML.NS':'Industrials',     'GRSE.NS':'Industrials',     'MAZAGON.NS':'Industrials',
  'DATAPATTNS.NS':'IT',        'KEC.NS':'Industrials',      'KALPATPOWR.NS':'Industrials',
  'GMRINFRA.NS':'Industrials', 'JSWINFRA.NS':'Industrials', 'ATGL.NS':'Energy',
  'CASTROLIND.NS':'Energy',    'MRPL.NS':'Energy',          'DELHIVERY.NS':'Industrials',
  'PAYTM.NS':'Financials',     'POLICYBZR.NS':'Financials',
}
...
const PRICES: Record<string, number> = {
  ...
  'HEROMOTOCO.NS':5200,'HINDALCO.NS':680, 'NESTLEIND.NS':24800,'ASIANPAINT.NS':2680,
  'ULTRACEMCO.NS':11200,'ONGC.NS':268,    'LTIM.NS':5400,     'PIDILITIND.NS':2850,
  'HAVELLS.NS':1640,  'TRENT.NS':5800,    'SIEMENS.NS':7200,  'ABB.NS':8400,
  'POLYCAB.NS':7100,  'ALKEM.NS':5400,    'TORNTPHARM.NS':3200,'BAJAJ-AUTO.NS':8900,
  'GODREJCP.NS':1240, 'MARICO.NS':620,    'VOLTAS.NS':1580,   'MUTHOOTFIN.NS':1980,
  'CHOLAFIN.NS':1380, 'NAUKRI.NS':7800,   'MRF.NS':145000,    'BOSCHLTD.NS':32000,
  'HAL.NS':4200,      'BEL.NS':290,       'PERSISTENT.NS':5800,'MPHASIS.NS':2800,
}
...
// ─── NIFTY 50 — 50 exact index constituents ──────────────────────────────────
const NIFTY50: Record<string, string> = {
  'RELIANCE.NS':'Energy',      'TCS.NS':'IT',               'HDFCBANK.NS':'Financials',
  'INFY.NS':'IT',              'ICICIBANK.NS':'Financials',  'BHARTIARTL.NS':'Telecom',
  'KOTAKBANK.NS':'Financials', 'SBIN.NS':'Financials',      'HINDUNILVR.NS':'FMCG',
...
// ─── NIFTY 200 extra — 83 stocks, none duplicated from NIFTY50 ───────────────
const NIFTY200_EXTRA: Record<string, string> = {
  'PIDILITIND.NS':'Consumer',  'HAVELLS.NS':'Consumer',     'SIEMENS.NS':'Industrials',
  'ABB.NS':'Industrials',      'POLYCAB.NS':'Industrials',  'DLF.NS':'Realty',
  'LODHA.NS':'Realty',         'GODREJCP.NS':'FMCG',        'MARICO.NS':'FMCG',
...
// ─── NIFTY 500 extra — 59 stocks, none duplicated from NIFTY200 ──────────────
const NIFTY500_EXTRA: Record<string, string> = {
  'KALYANKJIL.NS':'Consumer',  'TBOTEK.NS':'IT',            'MEDANTA.NS':'Pharma',
  'THELEELA.NS':'Consumer',    'AAVAS.NS':'Financials',     'HOMEFIRST.NS':'Financials',
  'DCBBANK.NS':'Financials',   'MAHINDCIE.NS':'Auto',       'SUNDRMFAST.NS':'Auto',
...
export async function fetchMarketPulse(): Promise<Record<string, number>> {
  ...
  await Promise.allSettled(['^NSEI', '^NSEBANK', '^BSESN', '^INDIAVIX', 'USDINR=X'].map(async sym => {
    try {
